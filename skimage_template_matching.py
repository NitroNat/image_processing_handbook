...
*** NOTE
Cross-correlation is similar to convolution, but without flipping the kernels
Convolution must be flipped L-R and T-B just like 1D convolution!
'''
...
# Load Image
image = data.coins()
# Create a template (or sub-image)
coin = image[170:220, 75:130]

# Apply Template Matching (raster 2D convolution)
...
ax2.add_patch(rect)
ax3.imshow(result) # result from cross-correlation
ax3.set_axis_off()
ax3.set_title('`match_template using cross-correlation`\nresult')
# highlight matched region
ax3.autoscale(False)
